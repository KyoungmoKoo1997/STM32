...
void SAI_TransmitReceive (uint32_t* Data, uint16_t DataSize){
	// uint32_t counter = 0;
	HAL_SAI_Transmit_IT_Revised(&hsai_BlockA1, (uint32_t*)Data, DataSize);
	HAL_SAI_Receive_IT_Revised(&hsai_BlockB1, (uint32_t*)rxData, DataSize);
...
void HAL_SAI_TxCpltCallback (SAI_HandleTypeDef *hsai){
	...
	if (switchFlag == 0){
		for(uint16_t i=0; i<100; i++){
			txData_current[i + (100 * visitor_count)] = txData[i + (100 * visitor_count)];
		}
	}else{
		for(uint16_t i=0; i<100; i++){
			txData_current[i + (100 * visitor_count)] = txData2[i + (100 * visitor_count)];
		}
	}
	visitor_count ++;
	if(visitor_count == 10){
...
void HAL_SAI_RxCpltCallback (SAI_HandleTypeDef *hsai){
	//UART_SendData(rxData, 100);
	static uint8_t visitor_count2 = 0;

		for(uint16_t i=0; i<100; i++){
			rxData2[i + (100 * visitor_count2)] = rxData[i + (100 * visitor_count2)];
						}
		visitor_count2 ++;
				if(visitor_count2 == 10){
					visitor_count2 = 0;
				}
}
...
/* Configure the peripherals common clocks */
  ...
  GenerateStepPositionData(655, txData, 1000);
  GenerateSinePositionData(txData2, 1000);
  ...
  SAI_TransmitReceive(txData_current, 1000);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  for(int i=0; i<5; i++){
	  	uint16_t xPos = (rxData2[i] >> 16) & 0xFFFF;
	  	uint16_t yPos = rxData2[i] & 0xFFFF;
