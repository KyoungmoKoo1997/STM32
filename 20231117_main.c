...
/** @defgroup SAI_Private_Functions  SAI Private Functions
  * @{
  */
static uint16_t SAI_FillFifo(SAI_HandleTypeDef *hsai);
...
static uint16_t SAI_FillFifo(SAI_HandleTypeDef *hsai)
{
  ...
  //{
    hsai->Instance->DR = *((uint32_t*)hsai->pBuffPtr + hsai->XferCount);
    hsai->XferCount = (hsai->XferCount + 1) % (hsai ->XferSize);
    n = (n+1) % 10;
  //}
  __HAL_SAI_ENABLE(hsai);
...
static void SAI_Receive_IT32Bit(SAI_HandleTypeDef *hsai)
{
	  ...
	  *((uint32_t*)hsai->pBuffPtr + hsai->XferCount) = (uint32_t) hsai->Instance->DR ;
	  hsai->XferCount = (hsai->XferCount + 1) % (hsai ->XferSize);
