  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  if(transmitFlag == 1){
		  ...
		  memcpy(txData, txStep, 500 * sizeof(uint32_t));
		  transmitFlag = 2;
	  }
	  else if (transmitFlag == 3){
		  //Sine Completely transmitted
		  memcpy(&txData[500], &txStep[500], 500 * sizeof(uint32_t));
		  transmitFlag = 4;
	  }
	  else if (transmitFlag == 5){
		  //Step half Transmitted
		  memcpy(txData, txSine, 500 * sizeof(uint32_t));
	  	  transmitFlag = 6;
	  }
	  else if (transmitFlag == 7){
		  //Step Completely Transmitted
		  memcpy(&txData[500], &txSine[500], 500 * sizeof(uint32_t));
	 	  transmitFlag = 0;
	  }
