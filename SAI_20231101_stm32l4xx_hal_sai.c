...
SAI_HandleTypeDef *hsai_private;
uint8_t *pData_private;
uint16_t Size_private;

SAI_HandleTypeDef *hsai_private2;
uint8_t *pData_private2;
uint16_t Size_private2;
...
HAL_StatusTypeDef HAL_SAI_Transmit_IT_Revised (SAI_HandleTypeDef *hsai, uint8_t *pData, uint16_t Size){
	 if ((pData == NULL) || (Size == 0U))
	  { return  HAL_ERROR;}

	  if (hsai->State == HAL_SAI_STATE_READY)
	  {
		uint16_t chunkSize = Size / 10;
		// Calculate new buffer position using modulo.
		//HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
	    //hsai->pBuffPtr = pData + ((hsai -> pBuffPtr - pData + chunkSize) % Size);
	    ...
	    // Transfer 32 bits in SAI Protocol
	    hsai->InterruptServiceRoutine = SAI_Transmit_IT32Bit;
	    SAI_FillFifo(hsai);

	    // Make the function interrupt driven - SAI
	    __HAL_SAI_ENABLE(hsai);
	    __HAL_SAI_ENABLE_IT(hsai, SAI_InterruptFlag(hsai, SAI_MODE_IT));
...
HAL_StatusTypeDef HAL_SAI_Receive_IT_Revised(SAI_HandleTypeDef *hsai, uint8_t *pData, uint16_t Size)
{
  if ((pData == NULL) || (Size == 0U))
  { return  HAL_ERROR; }
...
static void SAI_Transmit_IT32Bit(SAI_HandleTypeDef *hsai)
{
  int chunk_number = 100;
  int chunk_size = Size_private / chunk_number;
  if (hsai->XferCount == 0U)
  {
    /* Handle the end of the transmission */
    /* Disable FREQ and OVRUDR interrupts */
...
/*    hsai->State = HAL_SAI_STATE_READY;
    ...
    HAL_SAI_TxCpltCallback(hsai);
#endif */
	  hsai->pBuffPtr = pData_private + 4 * chunk_size * counter ;
	  hsai->XferSize = chunk_size;
	  hsai->XferCount = chunk_size;
...
{
  uint32_t temp;
  int chunk_number2 = 100;
  int chunk_size2 = Size_private2 / chunk_number2;
  if (counter2 == 0){
  hsai->XferSize = chunk_size2;
  hsai->XferCount = chunk_size2;
  hsai->pBuffPtr = pData_private2 + 4 * counter3 * chunk_size2 ; }
