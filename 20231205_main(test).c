...
void UART_SendData(uint32_t* Data, uint16_t DataSize){
	...
	sprintf(buffer, sizeof(buffer), "0x%08X\n", Data[i]);
	HAL_UART_Transmit(&huart2, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);
	}*/
	LED_Toggle();
	for (int i = 0; i < DataSize; i++){
	HAL_UART_Transmit(&huart2, (uint8_t*)&Data[i], 4, HAL_MAX_DELAY);
	}
}
...
void HAL_SAI_RxCpltCallback (SAI_HandleTypeDef *hsai){
	...
	HAL_SPI_Transmit_DMA(&hspi2, rxData, sizeof(rxData));
}
...
/* Configure the peripherals common clocks */
  ...
  GenerateStepPositionData(65, txData, 1000);
  ...
  sprintf(flag_buffer, "11111111111111111111111111111111");
  HAL_UART_Receive_DMA(&huart2, txData2, 500 * sizeof(uint32_t));
  HAL_UART_Transmit_DMA(&huart2, txData, 40);
  while (1)
  {
    /* USER CODE END WHILE */
	 //HAL_UART_Transmit(&huart2, txData, 40, HAL_MAX_DELAY);


	/*if (HAL_UART_Receive(&huart2, (uint8_t *)&received_data, 1, 1000) == HAL_OK) {
	      if (received_data == '1') {
	    	  while(1){
	    	  UART_SendData(txData, 1000);
	    	  UART_SendData(txSine, 1000);
	    	  }
	      }
  }*/
}
